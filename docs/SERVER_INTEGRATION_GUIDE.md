...
**Format:**
```
[0x03][Length][StringCount][String1_Len][String1_Data][String2_Len][String2_Data]...
```
...
[0x02]        // Number of strings
[0x04]        // String 1 length
['d','e','v','0']
[0x05]        // String 2 length
['6','0','6','0','7']
```

**Parsing Logic:**
```python
def parse_device_config(payload):
    if payload[0] != 0x03:
        raise ValueError("Invalid message type")
    
    length = payload[1]
    string_count = payload[2]
    ...
    for i in range(string_count):
        ...
        offset += 1
        string = payload[offset:offset+str_len].decode('utf-8')
        strings.append(string)
        offset += str_len
    
    return {
        'device_name': strings[0] if len(strings) > 0 else None,
        'zipcode': strings[1] if len(strings) > 1 else None
    }
```
...
**Encoding Logic:**
```python
def encode_current_weather(temp_fahrenheit):
    encoded_temp = int(temp_fahrenheit) + 50
    if encoded_temp < 0 or encoded_temp > 255:
        raise ValueError("Temperature out of range (-50 to +205°F)")
    return bytes([0x01, 0x01, encoded_temp])
```
...
**Encoding Logic:**
```python
def encode_forecast_weather(days):
    """
    days: list of dicts with keys 'high', 'precip_pct', 'moon_pct'
    """
    if len(days) < 1 or len(days) > 7:
        raise ValueError("Forecast must be 1-7 days")
    ...
    length = len(payload)
    return bytes([0x02, length] + payload)
```
...
def encode_version(version_number):
    if version_number < 0 or version_number > 255:
        raise ValueError("Version must be 0-255")
    return bytes([0x10, 0x01, version_number])
```
